...
- **Array fields still not registered as filterable.** `scripts/setup_vertex_search.py` now derives its `int_fields`/`array_fields` sets from `SchemaVocabulary` (the old `{"keywords", "entities"}` referenced the removed `entities` field and missed every psychotherapy array field), but the registration loop still `continue`s past array fields, leaving them unregistered. This contradicts `metadata_schema.json` `notes.vertex_ai_search`, which requires the 15 array fields registered as filterable attributes (type `key`/`text`). **Open** — the field-level indexing config must be expressed as annotations *inside* the `json_schema` string (`retrievable`/`indexable`/`searchable`/`dynamicFacetable` per property), **not** via a `discoveryengine.FieldConfig` object: no `FieldConfig` message and no `Schema.field_configs` field exist in the client library (verified absent from both `discoveryengine_v1` and `discoveryengine_v1beta` 0.20.0 — `Schema` carries only `name`/`struct_schema`/`json_schema`). See CHANGELOG 2026-07-09 (f); the existing `register_schema()` `FieldConfig` block is dead code that raises `AttributeError` regardless of API version.
